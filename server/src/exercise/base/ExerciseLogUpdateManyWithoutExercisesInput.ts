/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ExerciseLogWhereUniqueInput } from "../../exerciseLog/base/ExerciseLogWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";
@InputType()
class ExerciseLogUpdateManyWithoutExercisesInput {
  @Field(() => [ExerciseLogWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ExerciseLogWhereUniqueInput],
  })
  connect?: Array<ExerciseLogWhereUniqueInput>;

  @Field(() => [ExerciseLogWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ExerciseLogWhereUniqueInput],
  })
  disconnect?: Array<ExerciseLogWhereUniqueInput>;

  @Field(() => [ExerciseLogWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ExerciseLogWhereUniqueInput],
  })
  set?: Array<ExerciseLogWhereUniqueInput>;
}
export { ExerciseLogUpdateManyWithoutExercisesInput };
