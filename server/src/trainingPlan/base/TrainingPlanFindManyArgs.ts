/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TrainingPlanWhereInput } from "./TrainingPlanWhereInput";
import { Type } from "class-transformer";
import { TrainingPlanOrderByInput } from "./TrainingPlanOrderByInput";

@ArgsType()
class TrainingPlanFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => TrainingPlanWhereInput,
  })
  @Field(() => TrainingPlanWhereInput, { nullable: true })
  @Type(() => TrainingPlanWhereInput)
  where?: TrainingPlanWhereInput;

  @ApiProperty({
    required: false,
    type: [TrainingPlanOrderByInput],
  })
  @Field(() => [TrainingPlanOrderByInput], { nullable: true })
  @Type(() => TrainingPlanOrderByInput)
  orderBy?: Array<TrainingPlanOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { TrainingPlanFindManyArgs };
